...
// 5. Revenue Growth Calculation
$current_month_start = date('Y-m-01');
$last_month_start = date('Y-m-01', strtotime('-1 month'));
$last_month_end = date('Y-m-t', strtotime('-1 month'));

// Current Month Revenue
$curr_rev_query = "SELECT SUM(total_price) as total FROM orders WHERE status != 'cancelled' AND created_at >= '$current_month_start'";
$curr_revenue = $conn->query($curr_rev_query)->fetch_assoc()['total'] ?? 0;

// Last Month Revenue
$last_rev_query = "SELECT SUM(total_price) as total FROM orders WHERE status != 'cancelled' AND created_at >= '$last_month_start' AND created_at <= '$last_month_end'";
$last_revenue = $conn->query($last_rev_query)->fetch_assoc()['total'] ?? 0;
...
if ($last_revenue > 0) {
    $growth_percent = (($curr_revenue - $last_revenue) / $last_revenue) * 100;
    $revenue_growth = abs(round($growth_percent, 1));
    if ($growth_percent > 0) {
        $growth_class = 'text-success';
        $growth_icon = 'bi-arrow-up-short';
    } elseif ($growth_percent < 0) {
        $growth_class = 'text-danger';
        $growth_icon = 'bi-arrow-down-short';
    }
} else {
    // If no last month revenue, effectively 100% growth if we have revenue now
    if ($curr_revenue > 0) {
        $revenue_growth = 100;
        $growth_class = 'text-success';
        $growth_icon = 'bi-arrow-up-short';
...
<div class="row g-4 mb-4">
    <div class="col-md-3">
        <div class="antigravity-card p-4">
            ...
            <h3 class="fw-bold mb-0">PKR <?php echo number_format($total_revenue, 2); ?></h3>
            <small class="<?php echo $growth_class; ?>">
                <i class="bi <?php echo $growth_icon; ?>"></i> 
                <?php echo $revenue_growth > 0 ? $revenue_growth . '%' : 'No change'; ?> from last month
            </small>
        </div>
    </div>
    <div class="col-md-3">
