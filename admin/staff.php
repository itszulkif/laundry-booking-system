...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['staff_id']) && !empty($_POST['staff_id'])) {
        // Keep existing if invalid upload
        // We'll fetch existing avatar if no new one provided
        // But let's check input first
    }

    if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === 0) {
        $upload_dir = '../assets/img/'; // Relative to admin/staff.php
        $file_ext = strtolower(pathinfo($_FILES['avatar']['name'], PATHINFO_EXTENSION));
        $allowed_ext = ['jpg', 'jpeg', 'png', 'webp'];
        
        if (in_array($file_ext, $allowed_ext)) {
            $new_filename = uniqid('staff_', true) . '.' . $file_ext;
    ...
    try {
        if (isset($_POST['staff_id']) && !empty($_POST['staff_id'])) {
            ...
            // Only update avatar if new one uploaded
            if (isset($new_filename)) {
                $stmt = $conn->prepare("UPDATE staff SET name=?, email=?, phone=?, bio=?, status=?, working_start=?, working_end=?, working_days=?, daily_rate=?, avatar=? WHERE id=?");
                $stmt->bind_param("ssssssssdsi", $name, $email, $phone, $bio, $status, $working_start, $working_end, $working_days, $daily_rate, $avatar, $id);
            } else {
                $stmt = $conn->prepare("UPDATE staff SET name=?, email=?, phone=?, bio=?, status=?, working_start=?, working_end=?, working_days=?, daily_rate=? WHERE id=?");
                $stmt->bind_param("ssssssssdi", $name, $email, $phone, $bio, $status, $working_start, $working_end, $working_days, $daily_rate, $id);
            }
            $stmt->execute();
            ...
            // Insert
            $stmt = $conn->prepare("INSERT INTO staff (name, email, phone, bio, status, working_start, working_end, working_days, daily_rate, avatar) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssssds", $name, $email, $phone, $bio, $status, $working_start, $working_end, $working_days, $daily_rate, $avatar);
            $stmt->execute();
...
<div class="antigravity-card p-4 mb-4">
    <form method="GET" action="staff.php" class="row g-3">
        ...
        <div class="col-md-4">
            <label class="form-label small text-muted fw-bold">Filter by Zone</label>
                ...
                <?php if ($staff_result->num_rows > 0): ?>
                    <?php while($staff = $staff_result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <span class="fw-bold text-primary">PKR <?php echo number_format($staff['daily_rate'], 2); ?></span>
                            </td>
                            <td>
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
            ...
            <div class="modal-body p-4">
                <form method="POST" id="staffForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        <label class="form-label small text-muted fw-bold">Profile Picture</label>
                        <input type="file" name="avatar" class="form-control form-control-clean" accept="image/*">
                    </div>

                    <div class="mb-3">
                        <label class="form-label small text-muted fw-bold">Daily Rate (PKR)</label>
                        <input type="number" step="0.01" min="0" name="daily_rate" id="staffDailyRate" class="form-control form-control-clean" placeholder="0.00">
                    </div>

                    <div class="mb-3">
                        <label class="form-label small text-muted fw-bold">Assigned Zones</label>
                        ...
                        <div class="form-text small">Select zones where this staff member can work.</div>
                    </div>
...
function editStaff(staff) {
    ...
    document.getElementById('staffDailyRate').value = staff.daily_rate;
